...
export default class LoadAifKirov extends Loader {
    ...
    parse(inputData) {
        ...
        for(let item of inputDataArray) {
            ...
            if (begin) {
                if (itemArray[0] != 'Киров_Типография') {
                    let quantity = parseInt(itemArray[3]) + parseInt(itemArray[4]);
                    data.data.push({title: itemArray[0], value: quantity});
                    if(quantity) {
                        data.totalCount += quantity;
                    };                    
                }                
            }
